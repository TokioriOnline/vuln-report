...
MAJOR_PRODUCTS = [
    ...
    "f5","sharepoint","outlook","office","chrome",
    "firefox","adobe","log4j","spring","confluence",
    "gitlab","github","jenkins","netscaler","sonicwall"
]
MAJOR_THREATS = [
    "ransomware","remote code execution",
    "zero-day","0-day","unauthenticated"
]
...
def filter_kev_by_week(all_kev, week_start_str):
    start = datetime.strptime(week_start_str, "%Y-%m-%d")
    end = start + timedelta(days=7)
    return [
        v for v in all_kev
        if start <= datetime.strptime(v["dateAdded"], "%Y-%m-%d") < end
    ]
...
def get_css():
    return """
        ...
        .update-time { background: #e8f4f8; padding: 10px 15px;
                       border-radius: 5px; margin-bottom: 20px; }
        .disclaimer { background: #fff8e1;
...
def generate_feature_page(cve_id, product, vendor,
                          description, date_added,
                          due_date):
    os.makedirs(FEATURE_DIR, exist_ok=True)
    safe_id = cve_id.replace("/","-").replace(":","-").replace(" ","-")
    filepath = f"{FEATURE_DIR}/{safe_id}.html"

    product_lower = product.lower()
    desc_lower = description.lower()

    if "windows" in product_lower or "microsoft" in product_lower:
        impact = "Windows製品の脆弱性は中小企業への影響が特に広範です。社内PC・サーバー・リモートデスクトップ環境が攻撃対象となります。"
        mitigation = ["Windows Updateを即時実行する","自動更新が有効か全端末を確認","RDPの不要な公開を停止","ウイルス対策の定義ファイルを更新"]
    elif any(p in product_lower for p in ["cisco","fortinet","vpn","netscaler","ivanti","sonicwall"]):
        impact = "VPN機器・ネットワーク機器の脆弱性はリモートワーク環境で特に深刻です。社内ネットワーク全体が危険にさらされます。"
        mitigation = ["ファームウェアを即時更新","管理画面へのアクセスをIPアドレスで制限","不審な接続ログを確認","MFAを有効化"]
    elif any(p in product_lower for p in ["apache","nginx","tomcat","log4j"]):
        impact = "Webサーバーの脆弱性は自社Webサイトや社内システムへの不正アクセスにつながります。"
        mitigation = ["Webサーバーを最新版に更新","外部委託先に対応状況を確認","WAFの導入を検討","サーバーログに不審なアクセスがないか確認"]
    elif any(p in product_lower for p in ["exchange","outlook","office","sharepoint"]):
        impact = "メール・グループウェアの脆弱性はビジネスメール詐欺や情報漏洩の入口となります。"
        mitigation = ["Microsoft Updateを即時適用","メールフィルタリングの設定を強化","不審なメールの報告フローを周知","管理者アカウントのMFAを確認"]
    else:
        impact = "この脆弱性が自社システムで使用しているソフトウェアに該当しないか確認してください。"
        mitigation = ["影響を受けるバージョンを確認","ベンダーの公式情報でパッチを確認","パッチがない場合は回避策を適用","ITベンダーや専門家に相談"]

    ransomware_note = ""
    if "ransomware" in desc_lower:
        ransomware_note = '<div style="background:#fff0f0;border-left:4px solid #cc0000;padding:16px;border-radius:4px;margin:16px 0;"><h3 style="color:#cc0000;">⚠️ ランサムウェアによる悪用が報告されています</h3><p>バックアップがオフラインで保管されているか今すぐ確認してください。</p></div>'

    try:
        display_date = datetime.strptime(date_added,"%Y-%m-%d").strftime("%Y年%m月%d日")
    except Exception:
        display_date = date_added

    mitigation_html = "".join(f"<li style='padding:8px 0;border-bottom:0.5px solid #eee;'>✓ {m}</li>" for m in mitigation)

    html = f"""<!DOCTYPE html>
<html lang="ja">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>【特集】{cve_id} | Tokiori Online</title>
<style>{get_css()}</style></head>
<body>
<p><a href="../index.html">← 最新レポートに戻る</a> | <a href="../archive.html">📚 アーカイブ一覧</a></p>
<div style="background:#1a1a2e;color:white;border-radius:8px;padding:24px;margin-bottom:20px;">
<span style="background:#cc0000;color:white;padding:4px 12px;border-radius:12px;font-size:0.85em;font-weight:bold;display:inline-block;margin-bottom:12px;">🔴 CRITICAL 特集</span>
<h1 style="font-size:1.4em;margin-bottom:8px;">{cve_id}<br>{product} {"(" + vendor + ")" if vendor else ""}</h1>
<p>{description}</p>
<div style="font-size:0.85em;opacity:0.8;margin-top:8px;">📅 CISA KEV追加日: {display_date} {"　⏰ 対応期限: " + due_date if due_date else ""}</div>
</div>
{ransomware_note}
<div class="card"><h2>🏢 中小企業への影響</h2>
<div style="background:#fff8e1;border-left:4px solid #ffaa00;padding:16px;border-radius:4px;"><p>{impact}</p></div></div>
<div class="card"><h2>✅ 今すぐできる対策</h2><ul style="list-style:none;padding:0;">{mitigation_html}</ul></div>
<div class="card"><h2>🔗 一次情報源</h2>
<p><a href="https://www.cisa.gov/known-exploited-vulnerabilities-catalog" target="_blank">CISA KEV</a> / <a href="https://www.jpcert.or.jp/" target="_blank">JPCERT/CC</a> / <a href="https://jvndb.jvn.jp/" target="_blank">JVN iPedia</a></p></div>
...
def generate_archive_page(all_data, feature_vulns):
    ...
    for date in sorted(all_data.keys(), reverse=True):
        ...
        s = d["summary"]
        try:
            display = datetime.strptime(date,"%Y-%m-%d").strftime("%Y年%m月%d日")
        except Exception:
            display = date
        rows += f'<tr><td><a href="{date}.html">{display}</a></td><td style="color:#cc0000;"><strong>{s["critical"]}</strong></td><td style="color:#ff6600;">{s["high"]}</td><td style="color:#ffaa00;">{s["medium"]}</td><td style="color:#009900;">{s["low"]}</td><td>{s["total"]}</td></tr>'

    feature_rows = ""
    for v in feature_vulns:
        safe_id = v["id"].replace("/","-").replace(":","-").replace(" ","-")
        feature_rows += f'<tr><td><a href="feature/{safe_id}.html">📋 {v["id"]}</a></td><td>{v.get("product","")}</td><td>{v.get("vendor","")}</td><td>{v.get("dateAdded","")}</td></tr>'

    feature_section = ""
    if feature_rows:
        feature_section = f'<h2>📋 特集ページ一覧</h2><table><thead><tr><th>脆弱性ID</th><th>製品</th><th>ベンダー</th><th>KEV追加日</th></tr></thead><tbody>{feature_rows}</tbody></table><br>'
...
{feature_section}
<h2>📅 週次レポート一覧</h2>
<table><thead><tr><th>レポート日</th><th>🔴CRITICAL</th><th>🟠高</th><th>🟡中</th><th>🟢低</th><th>合計</th></tr></thead>
<tbody>{rows}</tbody></table>
<footer><p><strong>畠山正彦</strong> | ITセキュリティコンサルタント</p><p style="font-size:0.8em;color:#999;">© 2026 畠山正彦 All Rights Reserved.</p></footer>
</body></html>"""

    with open(f"{DOCS_DIR}/archive.html","w",encoding="utf-8") as f:
        f.write(html)
...
if __name__ == "__main__":
    ...
    vulns_hist = history.setdefault("vulnerabilities",{})

    for week_date in BACKFILL_DATES:
        print(f"\n{'='*40}")
        print(f"処理: {week_date}")
        week_kev = filter_kev_by_week(all_kev, week_date)
        print(f"該当KEV: {len(week_kev)}件")

        counts = {"CRITICAL":len(week_kev),"高":0,"中":0,"低":0}
        ...
        if week_date not in all_data:
            all_data[week_date] = {
                "date": week_date,
                "summary": {"critical":counts["CRITICAL"],"high":0,"medium":0,"low":0,"total":counts["CRITICAL"]},
                "kev": [{"cveID":v["cveID"],"product":v["product"],"vendorProject":v["vendorProject"],"description":v["shortDescription"],"dateAdded":v["dateAdded"],"dueDate":v["dueDate"]} for v in week_kev],
                "jpcert":[],"jvn":[]
            }
    ...
    print(f"✅ vuln_history.json 保存 ({len(vulns_hist)}件)")
